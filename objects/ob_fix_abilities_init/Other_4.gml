...
_elem[? "x"] = 0
_elem[? "y"] = 0
_elem[? "xs"] = frame_fix_ability.image_xscale
_elem[? "ys"] = frame_fix_ability.image_yscale
_elem[? "ob"] = frame_fix_ability.object_index
_elem[? "param"] = ""

fix_slot[0] = ds_map_create()
ds_map_copy(fix_slot[0], _elem)

with ob_ui_object
if place_meeting(x, y, frame_fix_ability) {
	_elem[? "x"] = x - frame_fix_ability.x
	_elem[? "y"] = y - frame_fix_ability.y
	_elem[? "xs"] = image_xscale
	_elem[? "ys"] = image_yscale
	_elem[? "ob"] = object_index
	_elem[? "param"] = parameter_name
	
	other.fix_slot[slot_elements_count] = ds_map_create()
	ds_map_copy(other.fix_slot[slot_elements_count], _elem)
	slot_elements_count++
}
...
while not is_undefined(_name) {
	if _name != "" and not is_undefined(bini_sections[? "abilities"][? _name]) {
		fixed_list[? _name] = bini_sections[? "abilities"][? _name]
		ds_map_delete(bini_sections[? "abilities"], _name)
	}
	_name = ds_map_find_next(ability_list, _name)
}
// refresh ability_list
ds_map_clear(ability_list)
_name = ds_map_find_first(fixed_list)
while not is_undefined(_name) {
	ds_map_add(ability_list, _name, true)
...
// bini_sections[? "abilities"] map contains remaining abilities that not represented in ability_list
var _list = sc_map_to_list(bini_sections[? "abilities"])
sc_create_slot_list(_list, fix_slot, frame_fix_ability.sprite_width, 1920 + 100, 50)
